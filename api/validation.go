...
var (
	...
	errInsufficientConfigServersNumber      = errors.New("sharding: minimum config servers number is 3")
	...
	errChangeShardsNumberNotSupported       = errors.New("sharding: change shards number is not supported")
	errChangeConfigServerNumberNotSupported = errors.New("sharding: change config server number is not supported")
...
func validateSharding(dbc DatabaseCluster) error {
	...
	if dbc.Spec.Sharding.ConfigServer.Replicas < minConfigServersNum {
		return errInsufficientConfigServersNumber
	}
...
func validateShardingOnUpdate(dbc *DatabaseCluster, oldDB *everestv1alpha1.DatabaseCluster) error {
	...
	if dbc.Spec.Sharding.Shards != oldDB.Spec.Sharding.Shards {
		return errChangeShardsNumberNotSupported
	}
	if dbc.Spec.Sharding.ConfigServer.Replicas != oldDB.Spec.Sharding.ConfigServer.Replicas {
		return errChangeConfigServerNumberNotSupported
	}
