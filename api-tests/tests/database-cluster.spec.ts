...
import {GetDatabaseClusterResponse} from "@support/types/database-clusters";

test.setTimeout(360 * 1000);

test.only('create db cluster with monitoring config', async ({ request, page }) => {
  ...
  const r: GetDatabaseClusterResponse = await postReq.json();
  r.items[]
