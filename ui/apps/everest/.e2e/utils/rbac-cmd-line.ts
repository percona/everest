...
export const restoreOldRBACPermissions = async () => {
  ...
  const command = `kubectl patch configmap/everest-rbac --namespace everest-system -p '{"data":${oldRbacFileContent}}'`;
  execSync(command);
...
export const setRBACPermissionsK8S = async (
  ...
  const command = `kubectl patch configmap/everest-rbac --namespace everest-system -p '{"data":{"enabled": "${permissions !== undefined}", "policy.csv":"g,${process.env.RBAC_USER},role:e2e-rbac-user\\n${permissions.map((p) => `p,role:e2e-rbac-user,${p.join(',')}`).join('\\n')}"}}'`;
  execSync(command);
