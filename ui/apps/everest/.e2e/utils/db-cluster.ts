...
export const getDbClusterAPI = async (clusterName: string, namespace: string, request: APIRequestContext, token: string) => {
...
export const updateDbClusterAPI = async (clusterName: string, namespace: string, payload: object, request: APIRequestContext, token: string) => {
...
export const getDbAvailableUpgradeVersionK8S = async (clusterName: string, namespace: string, request: APIRequestContext, token: string) => {
  ...
  const dbMajorVersion = dbType === "postgresql" ? dbSplitVersion[0] : dbSplitVersion[0]+'.'+dbSplitVersion[1];

  try {
    const response = await (await request.get(versionServiceURL+`/versions/v1/${dbOperatorName}/${crVersion}/${dbMajorVersion}-latest`)).json();
    ...
    const dbOperator = versions.find((item: any) => item.product === `${dbOperatorName}`);
