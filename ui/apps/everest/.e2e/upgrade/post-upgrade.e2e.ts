...
test.describe('Post upgrade tests', { tag: '@post-upgrade' }, async () => {
  ...
  test('Verify user is able to upgrade operators', async ({ page }) => {
    ...
    await test.step(`Upgrade CR version for each database`, async () => {
      await page.goto(`/settings/namespaces/${namespace}`);
      ...
      for (const operator of operatorsVersions) {
        ...
        await waitForStatus(page, `${operator.shortName}-db-cluster`, 'Initializing', TIMEOUTS.ThirtySeconds);
  ...
  test('Verify user is able to upgrade databases', async ({ page, request }) => {
    ...
    await test.step(`Upgrade each database with available upgrade`, async () => {
      for (const c of upgradeClustersInfo.filter((item) => item.upgradeVersion !== null)) {
        ...
        await waitForStatus(page, `${c.name}`, 'Initializing', TIMEOUTS.ThirtySeconds);
