...
import { deleteDbCluster, suspendDbCluster, resumeDbCluster, restartDbCluster } from '../utils/db-clusters-list';
...
[
  ...
  test.describe('Init deploy workflow', {
    ...
    test(`Cluster creation with ${db} and size ${size}`, async ({ page, request }) => {
      ...
      // basic info
      await populateBasicInformation(page, db, storageClasses[0], clusterName)
      await moveForward(page);

      // resources
      await page.getByRole('button').getByText(size+' node').click()
