  ...
  test('install only postgresql-operator', async ({ page, cli, request }) => {
    ...
    await test.step('run everest install command', async () => {
      const out = await cli.everestExecSkipWizard(
        `install -l --operator.mongodb=false --operator.postgresql=true --operator.xtradb-cluster=false --namespaces=everest-operators`,
      );
    ...
    await test.step('re-run everest install command', async () => {
      ...
      const out = await cli.everestExecSkipWizard(
        `install -l --operator.mongodb=false --operator.postgresql=true --operator.xtradb-cluster=true --namespaces=everest-operators`,
      );
