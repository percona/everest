...
export class CliHelper {
  ...
  async everestExecSkipWizard(command: string) {
    ...
    return test.step(`Run "${command}" command with --skip-wizard`, async () => {
      return this.execute(`${this.pathToBinary} ${command} --skip-wizard --version-metadata-url https://check-dev.percona.com`);
    });
  }
  ...
  async everestExecSkipWizardWithEnv(command, env: string) {
    ...
    return test.step(`Run "${command}" command with env variables`, async () => {
      return this.execute(`${env} ${this.pathToBinary} ${command} --skip-wizard --version-metadata-url https://check-dev.percona.com`);
    });
  }
