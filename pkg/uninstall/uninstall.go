...
func (u *Uninstall) deleteDBs(ctx context.Context) error {
	...
	return wait.PollUntilContextTimeout(ctx, 5*time.Second, 5*time.Minute, false, func(ctx context.Context) (bool, error) {
		...
		// See comment above - we sleep a bit to wait for the upstream operators to clean up all resources.
		u.l.Info("Databases have been removed. Waiting for the outstanding resources to be cleaned up.")
		time.Sleep(2 * time.Minute)
